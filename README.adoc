...
We use link:https://onsi.github.io/ginkgo/[Ginkgo] testing framework and link:link:https://onsi.github.io/gomega/[Gomega] assertion library for writing tests.
...
* In order to execute on test rename `It` function to `FIt`. Ginkgo runner will only execute given test (focused) instead of the whole test suite. This also works with `Context` and `Describe` blocks.
