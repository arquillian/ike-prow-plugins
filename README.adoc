...
plugins are based on link:https://github.com/kubernetes/test-infra/tree/master/prow[Prow] subproject of
...
* Add required secrets (see
* Apply `cluster/starter.yaml` with basic Prow infra.

The key piece of this deployment is `hook` service which listens to GitHub events and dispatch them to relevant plugins.
...
To speed up development cycle (and avoid pushing work-in-progress) images to Docker Hub you can use built-in docker repository
available in MiniShift. `Makefile` and templates are adjusted to use it, so the only thing you have to do is to set up
...
by `oc whoam -t`. To do that open Openshift Console (e.g. `xdg-open https://$(minishift ip):8443/console &>/dev/null'`),
...
IMPORTANT: You have to be logged in to cluster first.
