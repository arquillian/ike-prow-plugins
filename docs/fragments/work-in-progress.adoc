...
<2> Allows you to decide if you want to combine your patterns with predefined defaults (`true` by default).

=== Status details

==== Success [[wip-success]]

Your Pull Request has been approved as the plugin didn't detect any sign of work in progress (the PR title doesn't start with work-in-progress prefix).

==== Failed [[wip-failed]]

Your Pull Request has been rejected because the plugin detected that the PR title starts with work-in-progress prefix, so it seems that there is still an ongoing work.
