...
If, for whatever reason, you want to bypass this check - simply comment using `const:pkg/plugin/test-keeper/plugin/comment_cmd.go[name="SkipComment"]` command. If you are an admin user or requested PR reviewer but not a creator of the PR you will see the **Success** status.
...
If you are an admin and you are sure that no test is needed then you can use a command `const:pkg/plugin/test-keeper/plugin/comment_cmd.go[name="SkipComment"]` as a comment to make the status green.
