...
<4> Sets either relative path or absolute URL to a file that contains a plugin hint or an inline string comment to be added to the Pull Request when no test is found (if nothing is set then a link:https://github.com/arquillian/ike-prow-plugins/tree/master/pkg/plugin/test-keeper/plugin/comment_message.go[default message] is used).
