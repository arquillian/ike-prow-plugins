...
*:: wildcard for expressing "any name"

examples:

**/*_test.go:: matches any subdirectory containing a go file whose name ends with `_test`
*_test.go:: is same as the previous one - just shorter version
...
===== Regex
Don't use any regex expressions inside of the wildcard format. If the wildcard format is not suitable for you, you can still use the regex expressions, but it has to be surrounded by `regex{{...}}`. Both formats can be combined inside of the configured list of patterns - eg:
[source, yml, indent=0]
----
test_patterns:
  - '**/__test.go'
