...
<4> Sets plugin hint comment message to be added to the Pull Request when no test is found. It can be set either as a relative path or absolute URL to a file that contains a plugin hint and having case insensitive filename with an extension such as .md or .txt 
