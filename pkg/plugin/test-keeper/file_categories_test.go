...
var _ = Describe("Test fileCategoryCounter features", func() {
	...
	var defaultMatcher test_keeper.TestMatcher

	BeforeEach(func() {
		var err error
		defaultMatcher, err = test_keeper.LoadDefaultMatcher()
	...
	Context("Detecting tests within file changeset", func() {
		...
		It("should accept changeset containing Java file set when based on predefined matchers", func() {
			...
			fileCategoryCounter := test_keeper.FileCategoryCounter{Matcher: defaultMatcher}
		...
		It("should accept changeset based on configured inclusion", func() {
			// given
			matcher, loaderErr := test_keeper.LoadMatcher(test_keeper.TestKeeperConfiguration{
			...
			fileCategoryCounter := test_keeper.FileCategoryCounter{Matcher: matcher}
