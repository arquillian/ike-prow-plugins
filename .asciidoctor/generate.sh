...
function generateDoc() {
    echo "Generating doc from ${SOURCE} to ${TARGET}"

    docker run -v ${WORKING_DIR}:/docs/:Z -e PROJECT_ROOT=/docs/ --name adoc-to-html rochdev/alpine-asciidoctor:mini asciidoctor \
        -r /docs/.asciidoctor/lib/const-inline-macro.rb \
        -r /docs/.asciidoctor/lib/copy-to-clipboard-inline-macro.rb \
        -a generated-doc=true -a asciidoctor-source=/docs/docs ${ASCIIDOC_ARGS} \
        ${SOURCE} -o ${TARGET} --trace;
    ...
    if [ ${FAILED} -eq 1 ]; then
      echo "Failed generating the docs from ${SOURCE} to ${TARGET}"
    fi
}
...
if [ ${FAILED} -eq 1 ]; then
  echo "Failed generating the docs, see the errors above"
  exit 1
fi
